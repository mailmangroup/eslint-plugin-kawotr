...
const errorMessage = ( error, en, cn ) => {
	...
	switch ( error ) {
		...
		case 1:
			return `Missing Translation: Call to kawo.tr with an incorrect cn parameter: en --> ${en}`;

		case 2:
			return `Missing Translation: Call to kawo.tr with identical en and cn parameters: en --> ${en}`;

		case 3:
			return `Missing Translation: Call to kawo.tr with filler content: en --> ${en}`;
	}
};
