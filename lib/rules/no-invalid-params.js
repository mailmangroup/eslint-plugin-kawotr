...
const validateCall = ( context, node ) => {
	...
	// IF PASSED CALCULATED EXPRESSIONS › ASSUME CORRECT VALUE PASSED
	// (ALSO LET THROUGH IF NO CN VALUE GIVEN BUT EN IS BINARY OR MEMBER AND WE HAVE LOOKUP FUNCTION)
	if (
		( cn === 'ConditionalExpression' || !cn && en === 'ConditionalExpression' && typeof translationsLookup === 'function' ) ||
		( cn === 'MemberExpression' || !cn && en === 'MemberExpression' && typeof translationsLookup === 'function' ) ||
		( cn === 'BinaryExpression' || !cn && en === 'BinaryExpression' && typeof translationsLookup === 'function' )
	) return { node, error: 0 };
